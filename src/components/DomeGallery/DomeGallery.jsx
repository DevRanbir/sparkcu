...
import { getDomeGalleryImages, getStoredDomeGalleryImages } from '../../services/firebase';
...
export default function DomeGallery({
  ...
  useEffect(() => {
    const fetchDomeImages = async () => {
      try {
        const result = await getDomeGalleryImages();
        if (result.success && result.data && result.data.images && result.data.images.length > 0) {
          // Use manually configured images
          setDynamicImages(result.data.images);
        } else {
          // Fallback to stored images if no manual configuration
          const storedImages = await getStoredDomeGalleryImages();
          if (storedImages && storedImages.length > 0) {
            const fallbackImages = storedImages.map(img => ({
              src: img.url,
              alt: img.name || 'Gallery Image'
            }));
            setDynamicImages(fallbackImages);
          } else {
            // No images available, use empty array (will show defaults in buildItems function)
            setDynamicImages([]);
          }
        }
      } catch (error) {
  ...
  // Use prop images if provided, otherwise use dynamic images from Firebase
  const images = propImages || dynamicImages;
